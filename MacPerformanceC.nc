    ...
    if (!busy) 
        {
            ...
            #endif
            #if defined(TREE_TOPOLOGY_5)
            if (4 == TOS_NODE_ID)
            {        
                if (call AMSend.send(1, &pkt, sizeof(MacPerformanceMsg)) == SUCCESS) 
                {
                    mppkt->counter = my_id + counter;
                    counter++;
                    busy = TRUE;
            ...
            if (8 == TOS_NODE_ID)
            {        
                if (call AMSend.send(3, &pkt, sizeof(MacPerformanceMsg)) == SUCCESS) 
                {
                    mppkt->counter = my_id + counter;
                    counter++;
                    busy = TRUE;
